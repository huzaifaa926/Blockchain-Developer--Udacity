  ...
  submitStar(address, message, signature, star) {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      if (currentTime - messageTime >= 300)
        reject(new Error("More than 5 mins have elapsed!"));
      try {
        var isVerified = bitcoinMessage.verify(message, address, signature);
      } catch {
  ...
  getStarsByWalletAddress(address) {
    ...
    return new Promise((resolve, reject) => {
      self.chain.forEach(async (block) => {
        var blockData = await block.getBData();
    ...
    return new Promise(async (resolve, reject) => {
      self.chain.forEach(async (block) => {
        ...
        try {
          ...
          if (!isValid) errorLog.push("Invalid block hash");
          else {
            if (block.height > 0) {
              var prevBlock = self.chain.filter(
                (b) => b.height === block.height - 1
              )[0];
              if (block.previousBlockHash !== prevBlock.hash)
                errorLog.push("Invalid block link");
            }
          }
        } catch (error) {
          reject(error);
